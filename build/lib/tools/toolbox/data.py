...
from tools.app_config import Config, AppConfig
...
class Data():
    ...
    @staticmethod
    def log_media_exchange(source_barcode:str, destination_name:str, destination_barcode:str, source_wells:t.Iterable[str], percent_exchange:int, new_tips:bool) -> None:
        conf = Config()
        conf.load_app_config()
        ...
        file_folder= os.path.join(data_dir,"media_addition")
        if not os.path.exists(file_folder):
            os.mkdir(file_folder)
        log_file = os.path.join(file_folder, f"media_addition_{today}.txt")
        try:
            if os.path.exists(log_file) is False:
                with open(log_file, 'w+') as f:
                    f.write('source_barcode,destination_name,destination_barcode,destination_name,source_wells,percent_exchange,new_tips,media_exchange_on\n')
        except Exception as e:
            logging.debug(e)
            return
        
        with open(log_file, mode='a') as f:
            f.write(f'{source_barcode},{destination_name},{destination_barcode},{";".join(source_wells)},{percent_exchange},{new_tips},{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n')
