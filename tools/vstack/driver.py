...
if os.name == "nt":
    ...
    SDK_DLL = os.path.join(
                ...
                "AxInterop.StackerLib.dll",
            )

    clr.AddReference(SDK_DLL) 

    from AxStackerLib import AxStacker  # type: ignore

else:

    class AxStacker():  # type: ignore
        def __init__(self) -> None:
            pass
    
        def Initialize(self) -> None:
            raise NotImplementedError(
                "AxStacker is not supported on non-Windows platforms"
            )
...
class VStackDriver(ABCToolDriver):
    ...
    def set_labware(self, labware: str, plate_dimension_select: int) -> None:
        self.schedule_threaded_command("set_labware", {"labware": labware, "plate_dimension_select": plate_dimension_select})
        return 
    ...
    def execute_command(self, command: str, arguments: dict) -> None:
        ...
        try:
            if command == "initialize":
                ...
                response = self.client.SetLabware(arguments["labware"], arguments["plate_dimension_select"])
