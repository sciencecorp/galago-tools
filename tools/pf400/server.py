...
DEFAULT_PLATE_HANDLING_PARAMS: dict[
    str, dict[str, Union[Command.GraspPlate, Command.ReleasePlate]]
] = {
    "landscape": {
...
class Pf400Server(ToolServer):
    ...
    def __init__(self) -> None:
        ...
        self.grips: Grips
        self.plate_handling_params: dict[
            str, dict[str, Union[Command.GraspPlate, Command.ReleasePlate]]
        ] = {}
    ...
    def _getGrip(self, grip_name: str) -> Grip:
        grip = next(
            (x for x in self.grips.grip_params if x.name.lower() == grip_name.lower()), None
        )
        if not grip:
    ...
    def _getLocation(self, location_name: str) -> Optional[Location]:
        location = next(
            (x for x in self.waypoints.locations if x.name.lower() == location_name.lower()), None
        )
        if not location:
    ...
    def _getProfileId(self, profile_name: str) -> int:
        try:
            ...
            profile = next(
                (
                    x
                    for x in self.motion_profiles.profiles
                    if x.name.lower() == profile_name.lower()
                ),
                None,
            )
            if not profile:
            ...
            for grip in grip_params.grip_params:
                self.plate_handling_params[grip.name.lower()] = {
                    "grasp": Command.GraspPlate(
                        width=grip.width, force=grip.force, speed=grip.speed
                    ),
    ...
    def _unwind(self) -> None:
        ...
        if not location:
            raise KeyError(
                "Unwind location not found. Please add a unwind location to the waypoints. Height or rail position does not matter."
            )
    ...
    def moveTo(
        ...
        # For now we only handle a z offset for joints
            ...
            self.driver.movej(
                str(loc.coordinates + Coordinate(string_offset)), motion_profile=motion_profile_id
            )
        else:
    ...
    def retrieve_plate(
        ...
        if grip_width > 0:
            ...
            tmp_adjust_gripper = self.plate_handling_params[source_location.orientation.lower()][
                "release"
            ]
        ...
        pre_grip_sequence.append(
            Command.Move(
                location=source_location.name,
                motion_profile=motion_profile,
                approach_height=labware_offset,
            )
        )
    ...
    def dropoff_plate(
        ...
        if not release_params or (release_params.width == 0):
            ...
            tmp_release: Union[Command.GraspPlate, Command.ReleasePlate] = (
                self.plate_handling_params[dest_location.orientation.lower()]["release"]
            )
        ...
        dropoff_sequence.extend(
            [
                Command.Move(
                    ...
                    motion_profile=motion_profile,
                    approach_height=int(approach_height),
                ),  # Move to the dest location plus offset
                Command.Move(
                    location=dest_location.name,
                    motion_profile=motion_profile,
                    approach_height=labware_offset,
                ),  # Move to the nest location down in a straight pattern
                release,  # release the plate
            ]
        )

        post_dropoff_sequence.append(
            Command.Move(
                location=dest_location.name,
                motion_profile=motion_profile,
                approach_height=int(approach_height),
            )
        )  # Move to the approach offset

        self.runSequence(dropoff_sequence)
        self.driver.state.gripper_axis_override_value = (
            self._getGrip(dest_location.orientation).width + 10
        )
    ...
    def DropOffPlate(self, params: Command.DropOffPlate) -> None:
        self.dropoff_plate(
            destination_nest=params.location,
            motion_profile=params.motion_profile,
            approach_height=params.approach_height,
            labware_name=params.labware,
        )
        ...
        place_lid_sequence.extend(
            [
                Command.Move(
                    ...
                    motion_profile=motion_profile,
                    approach_height=int(labware.height + approach_height),
                ),  # Move to the location plus approach height
                Command.Move(
                    location=location.name,
                    motion_profile=motion_profile,
                    approach_height=int(lid_height),
                ),  # Move to the calculated height
                release,
            ]
        )

        post_place_sequence.append(
            Command.Move(
                location=location.name,
                motion_profile=motion_profile,
                approach_height=int(labware.height + approach_height),
    ...
    def RunSequence(self, params: Command.RunSequence) -> None:
        ...
        for arm_command in sequence.commands:
            ...
            if (
                command_name == "dropoff_plate"
                or command_name == "retrieve_plate"
                or command_name == "pick_lid"
                or command_name == "place_lid"
            ):
            ...
            try:
                ...
                logging.error(
                    f"Error parsing command {command_name} with params {command_params}: {e}"
                )
                raise Exception(
                    f"Error parsing command {command_name} with params {command_params}: {e}"
                )
