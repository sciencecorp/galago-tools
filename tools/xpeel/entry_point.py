#!/usr/bin/env python3
"""
XPeel Driver Entry Point

This script serves as the main entry point for the XPeel de-sealer gRPC server
...
def main() -> None:
    """Main entry point for the XPeel driver."""
    ...
    from tools.xpeel.server import XPeelServer
    from tools.base_server import serve
    
    # Get port from environment (controlled by Electron) or default
    port = os.environ.get("GRPC_PORT", "50051")
    
    logging.info(f"XPeel driver starting on port {port}")
    serve(XPeelServer(), port)
