#!/usr/bin/env python3
"""
PLR (PyLabRobot) Driver Entry Point

This script serves as the main entry point for the PLR gRPC server
...
def main() -> None:
    """Main entry point for the PLR driver."""
    ...
    from tools.plr.server import PLRToolServer
    from tools.base_server import serve
    
    # Get port from environment (controlled by Electron) or default
    port = os.environ.get("GRPC_PORT", "50051")
    
    logging.info(f"PLR driver starting on port {port}")
    serve(PLRToolServer(), port)
