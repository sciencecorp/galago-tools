...
# Fix Windows encoding issues - must be at the top!
if os.name == 'nt':
    ...
    try:
        ...
        # If UTF-8 setup fails, we'll fall back to safe characters later
        pass
...
def get_emoji_safe_text(emoji: str, fallback: str) -> str:
    """
    Returns emoji if platform supports it, otherwise returns fallback text
    """
    try:
        # Test if we can actually print the emoji
        test_output = io.StringIO()
        print(emoji, file=test_output, end='')
        test_output.getvalue()
        
        # Additional check for Windows environments
        if (os.name == 'nt' and 
            ('TERM_PROGRAM' not in os.environ or 
             sys.stdout.encoding.lower() in ['cp1252', 'cp437'])):
            return fallback
        return emoji
...
ROCKET = get_emoji_safe_text("🚀", "[ROCKET]")
ROBOT = get_emoji_safe_text("🤖", "[ROBOT]")
PACKAGE = get_emoji_safe_text("📦", "[PACKAGE]")
CLOCK = get_emoji_safe_text("⏰", "[TIME]")
ID_BADGE = get_emoji_safe_text("🆔", "[ID]")
COMPUTER = get_emoji_safe_text("💻", "[SYSTEM]")
FOLDER = get_emoji_safe_text("📂", "[FOLDER]")
CHECK = get_emoji_safe_text("✅", "[OK]")
REFRESH = get_emoji_safe_text("🔄", "[LOADING]")
...
class ToolsManager():
    ...
    def display_startup_message(self) -> None:
        ...
        # Startup banner with better formatting - using safe emojis
        self.log_text("=" * 80)
        self.log_text(f"{ROCKET} {ROBOT} GALAGO TOOLS MANAGER STARTED", "success")
        self.log_text("=" * 80)
        self.log_text("")
        
        # Version and system info
        self.log_text(f"{PACKAGE} Version: {galago_version}", "info")
        if UPDATE_AVAILABLE:
        ...
        self.log_text(f"\n{CLOCK} Started: {current_time}\n", "info")
        self.log_text(f"{ID_BADGE} Session: {LOG_TIME}\n", "info")
        self.log_text(f"{COMPUTER} Platform: {os.name}\n", "info")
        
        # URLs and important info
        self.log_text(f"{FOLDER} URLs:", "header")
        ...
        # Status message
        self.log_text(f"{CHECK} Manager initialized successfully\n", "success")
        self.log_text(f"{REFRESH} Starting tool servers...\n", "info")
        self.log_text("-" * 80)
        self.log_text("")
...
def main() -> int:
    ...
    try:
        ...
        # Use safe emojis for console output
        safe_print("=" * 80)
        safe_print(f"{ROCKET} {ROBOT} GALAGO TOOLS MANAGER STARTED")
        safe_print("=" * 80)
        safe_print("")
        safe_print(f"{PACKAGE} Version: {galago_version}")
        safe_print(f"{CLOCK} Started: {current_time}")
        safe_print(f"{ID_BADGE} Session: {LOG_TIME}")
        safe_print(f"{COMPUTER} Platform: {os.name}")
        safe_print("")
        safe_print(f"{FOLDER} URLs:")
        safe_print(f"   Tool Server Ip: {LOCAL_IP}")
        safe_print("   Galago Web Local: http://localhost:3010/")
        safe_print(f"   Galago Web On network: http://{LOCAL_IP}:3010/")
        safe_print("")
        safe_print(f"{CHECK} Manager initialized successfully")
        safe_print(f"{REFRESH} Starting tool servers...")
        safe_print("-" * 80)
        safe_print("")
